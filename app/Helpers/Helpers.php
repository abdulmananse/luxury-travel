...
if (!function_exists('sendSms')) {
    ...
    function sendSms($number, $message, $cnic = null, $type = null, $logId = 0)
    {
        ...
        //     $cnic = (int) $cnic;
        //     $part = (int) substr($cnic, -3);
        //     $mobile = $number;

        //     $applicant = Applicant::where('cnic', $cnic)
        //                     ->where('part', $part)
        //                     ->where(function($query) use ($mobile) {
        //                         $query->where('sms_mobile', $mobile)
        //                             ->orWhere('user_mobile', $mobile);
        //                     })
        //                     ->getFirst(); 

        //     if ($applicant) {
        //         if ($applicant->is_sent_nadra_beneficiary == 1) {
        //             if(sendSms($mobile, __('messages.eligible_beneficary_sms', ['cnic' => $applicant->cnic, 'mobile' => $mobile]), $applicant->cnic, 'eligible')) {
        //                 $applicant->sms_status = 1;
        //                 $applicant->save();
        //                 exit;
        //             }
        //         } else if ($applicant->nadra_eligibility_status == 2) {
        //             if(sendSms('03334632943', __('messages.nadra_ineligible_beneficary', ['cnic' => $applicant->cnic]), $applicant->cnic, 'nadra_ineligible')) {
        //                 $applicant->sms_status = 3;
        //                 $applicant->save();
        //                 exit;
        //             }
        //         }
...
* @return mix
*/
if (!function_exists('createCaptchaSession')) {
    function createCaptchaSession()
    {
        Session::put('captcha_1', rand(1, 9));
        Session::put('captcha_2', rand(1, 9));
        return true;
    }
}
...
* @return mix
*/
if (!function_exists('getCaptchaTotal')) {
    function getCaptchaTotal()
    {
        return Session::get('captcha_1') + Session::get('captcha_2');
    }
}
...
if (!function_exists('showCaptcha')) {
    function showCaptcha()
    {
        return !in_array(
            request()->ip(), 
            explode(',', config('app.google_recaptcha_bypass_ips'))
        );
    }
}
